...
public class SigantureTestPROD {
	...
	public static void main(String[] args) throws Exception {
    	...
    	X509Certificate inter = parseCertificate(IOUtils.toString(SigantureTestPROD.class.getResourceAsStream("/prod/inter.pem")));
    	X509Certificate root = parseCertificate(IOUtils.toString(SigantureTestPROD.class.getResourceAsStream("/prod/root.pem")));
	...
	public static boolean verifySignature(byte[] data, byte[] signature, KeyStore ks) throws Exception {
        ...
        for (Object o : signerInformationStore.getSigners()) {
            ...
            if (!matches.isEmpty()) {
                ...
                if (signer.verify(new JcaSimpleSignerInfoVerifierBuilder().setProvider(BouncyCastleProvider.PROVIDER_NAME).build(certificateHolder))) {
                    verified = true;
                }
            }
        }
